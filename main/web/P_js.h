...
function load_clutch(){
  if(xmlHttp.readyState==0 || xmlHttp.readyState==4){
    xmlHttp.open('GET','/web/clutch.json',true);
    xmlHttp.send(null);
    xmlHttp.onload = function(e) {
      jsonResponse=JSON.parse(xmlHttp.responseText);
      loadBlock(xmlHttp.onload);
...
function updBlock() {
  ...
  for (var key in data2) {
    ...
    if (input2 != null) {
      if (input2.type == 'number' || input2.type == 'text') {
        input2.value = data2[key];
      }
      if (input2.type == 'checkbox') {
        input2.checked = data2[key] == 'checked' ? true : false;
      }
      else {
