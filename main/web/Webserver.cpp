...
void Webserver::init() {
    ...
    httpd_uri_t unit_info = {
        .uri = "/unit-info.json",
        .method = HTTP_GET,
        .handler = unit_info_handler,
        .user_ctx = NULL
    };
    httpd_register_uri_handler(server, &unit_info);
...
esp_err_t Webserver::favicon_handler(httpd_req_t *req) {
    ...
    httpd_resp_set_hdr(req, "Last-Modified", "Sat, 11 Mar 2023 10:00:00 GMT");
    httpd_resp_set_hdr(req, "Cache-Control", "max-age=2592000");
    httpd_resp_send(req, (char*)favicon_ico, sizeof(favicon_ico));
    return ESP_OK;
}

esp_err_t Webserver::unit_info_handler(httpd_req_t *req) {
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, UnitInfo::getJSON().c_str(), HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
void Webserver::registerPage(Page* page) {
    httpd_uri_t webPage = {
        .uri = strdup(page->getUrl()),
        .method = HTTP_GET,
        .handler = page->webPageHandler,
        .user_ctx = page
    };
